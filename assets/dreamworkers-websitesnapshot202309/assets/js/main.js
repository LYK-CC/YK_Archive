...
(function() {
  ...
  window.addEventListener('load', () => {
    ...
    /**
   * Tutor isotope and filter
   */
  window.addEventListener('load', () => {
    let tutorContainer = select('.tutor-container');
    if (tutorContainer) {
      let tutorIsotope = new Isotope(tutorContainer, {
        itemSelector: '.tutor-item',
        layoutMode: 'fitRows'
      });

      let tutorFilters = select('#tutor-flters li', true);

      on('click', '#tutor-flters li', function(e) {
        e.preventDefault();
        tutorFilters.forEach(function(el) {
          el.classList.remove('filter-active');
        });
        this.classList.add('filter-active');

        tutorIsotope.arrange({
          filter: this.getAttribute('data-filter')
        });
      }, true);
    }
